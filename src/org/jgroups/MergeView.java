...
/**
 * A view that is sent as a result of a cluster merge. Whenever a group splits into subgroups, e.g.,
 * due to a network partition, and later the subgroups merge back together, a MergeView instead of a
 * View will be received by the application. The MergeView class is a subclass of View and contains
 * as additional instance variable: the list of views that were merged. For example, if the group
...
public class MergeView extends View {
    ...
    public boolean deepEquals(View other) {
        ...
        if (subgroups == null && other_view.subgroups == null)
            return true;
        assert subgroups != null;
        if (subgroups.length != other_view.subgroups.length)
            return false;
    ...
    private static View[] listToArray(List<View> list) {
        if (list == null)
            return null;
