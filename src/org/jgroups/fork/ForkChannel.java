...
public class ForkChannel extends JChannel implements ChannelListener {
    ...
    /**
     * Creates a new fork-channel from a main-channel. The channel is unconnected and {@link
     * ForkChannel#connect(String)} needs to be called to send and receive messages.
     *
     * @param main_channel The main-channel. The lifetime of the newly created channel will be less
     * than or equal to the main-channel
     * @param fork_stack_id The ID to associate the fork-stack with in FORK
     * @param fork_channel_id The ID used to map fork-channel IDs to ForkChannels in the
     * fork-channels protocol stack
     * @param create_fork_if_absent If true, and FORK doesn't exist, a new FORK protocol will be
     * created and inserted into the main-stack at the given position. If false, and FORK doesn't
     * exist, an exception will be thrown
     * @param position The position at which the newly created FORK will be inserted. {@link
     * ProtocolStack.Position#ABOVE} or {@link ProtocolStack.Position#BELOW} are accepted. Ignored
     * if create_fork_if_absent is false.
     * @param neighbor The class of the neighbor protocol below or above which the newly created
     * FORK protocol will be inserted. Ignored if create_fork_if_absent is false.
     * @param protocols A list of protocols (<em>from bottom to top</em> !) to insert as the
     * fork_stack in FORK under the given fork_stack_id. If the fork-stack with fork_stack_id
     * already exists, an exception will be thrown. Can be null if no protocols should be added.
     * This may be the case when an app only wants to use a ForkChannel to mux/demux messages, but
     * doesn't need a different protocol stack.
     * @throws Exception
     */
    public ForkChannel(final JChannel main_channel, String fork_stack_id, String fork_channel_id,
        boolean create_fork_if_absent, ProtocolStack.Position position,
    ...
    /**
     * Creates a new fork-channel from a main-channel. The channel is unconnected and {@link
     * ForkChannel#connect(String)} needs to be called to send and receive messages. If FORK is not
     * found in the stack, an exception will be thrown.
     *
     * @param main_channel The main-channel. The lifetime of the newly created channel will be less
     * than or equal to the main-channel
     * @param fork_stack_id The ID to associate the fork-stack with in FORK
    ...
    /**
     * Connects the fork-channel, which will be operational after this. Note that the fork-channel
     * will have the same state as the main-channel, ie. if the main-channel is disconnected, so
     * will the fork-channel be, even if connect() was called. This connect() method essentially
     * adds the fork-channel to the fork-stack's hashmap, ready to send/receive messages as soon as
     * the main-channel has been connected.<p/> This method does <em>not</em> affect the
     * main-channel.
     *
    ...
    /**
     * Removes the fork-channel from the fork-stack's hashmap and resets its state. Does
     * <em>not</em> affect the main-channel
     */
    @Override
    ...
    /**
     * Closes the fork-channel, essentially setting its state to CLOSED. Note that - contrary to a
     * regular channel - a closed fork-channel can be connected again: this means re-attaching the
     * fork-channel to the main-channel
     */
    @Override
    public void close() {
        ((ForkProtocolStack) prot_stack).remove(fork_channel_id);
    ...
    @Override
    public ForkChannel startFlush(List<Address> flushParticipants,
    ...
    /**
     * Creates a new FORK protocol, or returns the existing one, or throws an exception. Never
     * returns null.
     */
    protected static FORK getFORK(JChannel ch, ProtocolStack.Position position,
        Class<? extends Protocol> neighbor,
