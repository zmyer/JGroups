...
public class UDP extends TP {
    ...
    public void sendUnicast(PhysicalAddress dest, byte[] data, int offset,
        int length) throws Exception {
        _send(((IpAddress) dest).getIpAddress(), ((IpAddress) dest).getPort(), data, offset, length);
    }

    protected void _send(InetAddress dest, int port, byte[] data, int offset,
        int length) throws Exception {
    ...
    /**
     * Creates a DatagramSocket when bind_port > 0. Attempts to allocate the socket with port ==
     * bind_port, and increments until it finds a valid port, or until port_range has been exceeded
     *
