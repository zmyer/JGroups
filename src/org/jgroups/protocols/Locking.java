...
/**
 * Base locking protocol, handling most of the protocol communication with other instances. To use
 * distributed locking, {@link org.jgroups.blocks.locking.LockService} is placed on a channel.
 * LockService talks to a subclass of Locking via events.
 *
 * @author Bela Ban
...
abstract public class Locking extends Protocol {
    ...
    abstract protected void sendGrantLockRequest(String lock_name, int lock_id, Owner owner,
        long timeout, boolean is_trylock);
    ...
    protected void sendRequest(Address dest, Type type, String lock_name, Owner owner, long timeout,
        boolean is_trylock) {
        send(dest, new Request(type, lock_name, owner, timeout, is_trylock));
    }

    protected void sendRequest(Address dest, Type type, String lock_name, int lock_id, Owner owner,
        long timeout, boolean is_trylock) {
     ...
     * The semantics are more or less those of {@link Lock}, but may differ slightly. For details
     * see {@link org.jgroups.blocks.locking.LockService}.
     */
    protected class ClientLock implements Lock {
        ...
        protected synchronized boolean acquireTryLock(long timeout,
            boolean use_timeout) throws InterruptedException {
            if (denied)
                return false;
            if (!acquired) {
    ...
    protected class ClientLockTable {
        ...
        protected synchronized ClientLock getLock(String name, Owner owner,
