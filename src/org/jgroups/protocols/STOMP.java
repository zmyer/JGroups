...
/**
 * Protocol which provides STOMP (http://stomp.codehaus.org/) support. Very simple implementation,
 * with a one-thread-per-connection model. Use for a few hundred clients max.<p/> The intended use
 * for this protocol is pub-sub with clients which handle text messages, e.g. stock updates, SMS
 * messages to mobile clients, SNMP traps etc.<p/> Note that the full STOMP protocol has not yet
 * been implemented, e.g. transactions are not supported. todo: use a thread pool to handle incoming
 * frames and to send messages to clients
 * <p/>
...
public class STOMP extends Protocol implements Runnable {
    ...
    private String getAddress() {
        ...
        for (Util.AddressScope scope : Util.AddressScope.values()) {
            try {
                ...
                if (addr != null)
                    return addr.getHostAddress() + ":" + srv_sock.getLocalPort();
    ...
    public class Connection implements Runnable {
        ...
        /**
         * Sends back a response. The keys_and_values vararg array needs to have an even number of
         * elements
         *
         * @param response
    ...
    public static class StompHeader extends org.jgroups.Header {
         ...
         *
         * @param type
         * @param headers Keys and values to be added to the header hashmap. Needs to be an even
         * number
         * @return
         */
