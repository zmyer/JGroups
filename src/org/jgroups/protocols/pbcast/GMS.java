...
public class GMS extends Protocol implements DiagnosticsHandler.ProbeHandler {
   ...
   protected boolean _startFlush(final View new_view, int maxAttempts, boolean resumeIfFailed, long randomFloor, long randomCeiling) {
            ...
            case Event.MSG:
                ...
                switch(hdr.type) {
                    ...
                    case GmsHeader.JOIN_RSP:
                        JoinRsp join_rsp=readJoinRsp(msg.getBuffer());
                        if(join_rsp != null)
                    ...
                    case GmsHeader.LEAVE_RSP:
                        ...
                        break;
                    case GmsHeader.VIEW:
                        Tuple<View,Digest> tuple=readViewAndDigest(msg.getBuffer());
                        if(tuple == null)
                            return null;
                    ...
                    case GmsHeader.MERGE_REQ:
                        Collection<? extends Address> mbrs=readMembers(msg.getBuffer());
                        if(mbrs != null)
                            impl.handleMergeRequest(msg.getSrc(), hdr.merge_id, mbrs);
                        break;

                    case GmsHeader.MERGE_RSP:
                        tuple=readViewAndDigest(msg.getBuffer());
                        if(tuple == null)
                            return null;
    ...
    protected static Buffer marshal(final View view, final Digest digest) {
        ...
        short flags=determineFlags(view, digest);
        try {
    ...
    protected JoinRsp readJoinRsp(byte[] buffer) {
        try {
            return buffer != null? Util.streamableFromBuffer(JoinRsp.class, buffer) : null;
        }
        catch(Exception ex) {
    ...
    protected Collection<? extends Address> readMembers(byte[] buffer) {
