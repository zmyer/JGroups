 ...
 * @author Bela Ban
 * @version $Id: GMS.java,v 1.126.2.25 2009/12/15 17:08:30 belaban Exp $
 */
        ...
        synchronized(members) {   // serialize access to views
            ...
            if(coord != null && coord.equals(local_addr) && !haveCoordinatorRole()) {
                ...
                if(haveCoordinatorRole() && !local_addr.equals(coord)) {
                    becomeParticipant();
                    merge_ack_collector.reset(null);
                }
            }
        }
    }
...
//    }
    ...
    public boolean setProperties(Properties props) {
        ...
        str=props.getProperty("flush_invoker_class");
        if (str != null) {
			try {
				flushInvokerClass = (Class<Callable<Boolean>>) Class.forName(str);
				flushInvokerClass.getDeclaredConstructor(View.class);
			} catch (Exception e) {
				log.error("Invalid flush invoker class "
								+ str
								+ ". Maker sure it implements <Callable<Boolean>> with public constructor using View as a parameter");
			}
			props.remove("flush_invoker_class");
		}
    ...
    class ViewHandler implements Runnable {
        ...
        synchronized void stop(boolean flush) {
            ...
            // commented as this is already done in waitUnitlCompleted()
            // thread = null; // Workaround for potential classloader leak to our thread
        }
    }
