...
public class FLUSH extends Protocol {
    ...
    private void blockMessageDuringFlush() {
        ...
        if (shouldSuspendByItself) {
            log.warn("unblocking FLUSH.down() at " + localAddress + " after timeout of "
                            + (stop - start) + "ms");
                    ...
                    switch (fh.type) {
                        ...
                        case FlushHeader.START_FLUSH:
                            ...
                            if (amIParticipant) {
                                ...
                                if (log.isDebugEnabled())
                                    log.debug("Received START_FLUSH at " + localAddress + " but I am not flush participant, not responding");
                            }
                            break;
        ...
        if (proceed) {
            ...
            if (log.isDebugEnabled())
                log.debug("Received START_FLUSH at " + localAddress
                                + " responded with FLUSH_NOT_COMPLETED to " + flushRequester);
        }
    }
    ...
    private boolean onViewChange(View view) {
        ...
        synchronized (sharedLock) {
            ...
            currentView = view;
            coordinatorLeft = !view.getMembers().isEmpty()
                            && !view.containsMember(view.getCreator());
        }
        if (log.isDebugEnabled())
    ...
    private void onSuspect(Address address) {
        ...
        if (amINeighbourOfCrashedFlushCoordinator) {
            if (log.isDebugEnabled())
                log.debug("Flush coordinator " + flushCoordinator + " suspected, " + localAddress
                                + " is neighbour, completing flush ");
