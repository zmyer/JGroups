 ...
 * conjuntion with a non-UDP transport, e.g. TCP.<p> The discovery is <em>asymmetric</em>: discovery
 * requests are broadcast via the multicast socket, and received via the multicast socket by
 * everyone in the group. However, the discovery responses are sent back via the regular transport
 * (e.g. TCP) to the sender (discovery request contained sender's regular address, e.g.
 * 192.168.0.2:7800).
 *
 * @author Bela Ban
 */
public class MPING extends PING implements Runnable {
    ...
    /**
     * List<NetworkInterface> of interfaces to send multicasts on. The multicast send socket will
     * send the same multicast message on all of these interfaces. This is a comma-separated list of
     * IP addresses or interface names. E.g. "192.168.5.1,eth1,127.0.0.1". Duplicates are discarded.
    ...
    /**
     * If we have multiple mcast send sockets, e.g. send_interfaces or send_on_all_interfaces
     * enabled
     */
    ...
    protected void bindToInterfaces(List<NetworkInterface> interfaces, MulticastSocket s,
        InetAddress mcast_addr) throws IOException {
