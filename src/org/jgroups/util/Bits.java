...
/**
 * Class (similar to (and partly copied from) java.nio.Bits) containing helper methods to encode
 * variables (e.g. ints, long, List&lt;Address&gt; etc) to memory (byte buffer) or output streams
 * and read variables from memory or input streams.<p/> The write methods write a type (e.g. an int
 * or a char) to a buffer ({@link ByteBuffer} or output stream, using <a
 * href="https://developers.google.com/protocol-buffers/docs/encoding">variable-length encoding</a>.
 * If there are not enough byte in the buffer to write a type, a {@link
 * java.nio.BufferOverflowException} is thrown. If the variable cannot be written to the output
 * stream, an IOException is thrown.
 * <p/>
 * The read methods read a variable-length encoded type from a buffer or input stream. If there are
 * fewer bytes in the buffer than needed to read the type, a {@link java.nio.BufferUnderflowException}
 * is thrown. If the read fails, an IOException is thrown.
 * <p/>
 * The size() methods return the number of bytes used to encode the given type with variable-length
 * encoding.
 * <p/>
 * There are additional helper methods to write/read custom JGroups types, e.g. address lists, Views
 * etc
 * <p/>
 * Note that methods to read/write atomic types (char, int etc) should only be used if
 * variable-length encoding is desired; otherwise {@link DataOutput#writeInt(int)} or {@link
 * ByteBuffer#putInt(int)} should be used instead.
 * <p/>
...
public final class Bits {
    ...
    private static char makeChar(byte[] buf, int offset) {
        return (char) ((buf[offset + 1] & 0xFF) + (buf[offset] << 8));
    }

    static void writeChar(char c, byte[] buf, int offset) {
        buf[offset + 1] = (byte) c;
        buf[offset] = (byte) (c >>> 8);
    }

    static char readChar(byte[] buf, int offset) {
    ...
    /**
     * Writes a long to out in variable-length encoding. Note that currently variable-length
     * encoding is <em>not</em> used (a similar mechanism is used); this will be implemented later.
     *
     * @param num the long
    ...
    /**
     * Reads a variable-length encoded long from an input stream.  Note that currently
     * variable-length encoding is <em>not</em> used (a similar mechanism is used); this will be
     * implemented later.
     *
     * @param in the input stream
    ...
    /**
     * Computes the size of a variable-length encoded long.  Note that this is <em>not</em>
     * currently using variable-length encoding (will be implemented later).
     *
     * @param num the long
     * @return the number of bytes needed to variable-length encode num
     */
    ...
    /**
     * Writes 2 sequence numbers (seqnos) in compressed format to buf. The seqnos are non-negative
     * and hr is guaranteed to be &gt;= hd.
     * <p/>
     * Once variable-length encoding has been implemented, this method will probably get dropped as
     * we can simply write the 2 longs individually.
     *
     * @param hd the highest delivered seqno. Guaranteed to be a positive number
     * @param hr the highest received seqno. Guaranteed to be a positive number. Greater than or
     * equal to hd
    ...
    /**
     * Writes 2 sequence numbers (seqnos) in compressed format to an output stream. The seqnos are
     * non-negative and hr is guaranteed to be &gt;= hd.
     * <p/>
     * Once variable-length encoding has been implemented, this method will probably get dropped as
     * we can simply write the 2 longs individually.
     *
     ...
     * <p/>
     * Once variable-length encoding has been implemented, this method will probably get dropped as
     * we can simply read the 2 longs individually.
     *
     ...
     * @param seqnos the array to read the seqnos into, needs to have a length of 2
     * @param index the index of the first element to be written; the seqnos are written to
     * seqnos[index] and seqnos[index+1]
     */
    ...
    private static int makeInt(byte[] buf, int offset, int bytes_to_read) {
        int retval = 0;
    ...
    private static int makeInt(ByteBuffer buffer, int bytes_to_read) {
        int retval = 0;
        for (int i = 0; i < bytes_to_read; i++) {
            byte b = buffer.get();
            retval |= ((int) b & 0xff) << (i * 8);
    ...
    private static int makeInt(DataInput in, int bytes_to_read) throws IOException {
        int retval = 0;
        for (int i = 0; i < bytes_to_read; i++) {
            byte b = in.readByte();
            retval |= ((int) b & 0xff) << (i * 8);
    ...
    private static long makeLong(byte[] buf, int offset, int bytes_to_read) {
        long retval = 0;
        for (int i = 0; i < bytes_to_read; i++) {
            byte b = buf[offset + i];
    ...
    private static long makeLong(ByteBuffer buffer, int bytes_to_read) {
        long retval = 0;
        for (int i = 0; i < bytes_to_read; i++) {
            byte b = buffer.get();
            retval |= ((long) b & 0xff) << (i * 8);
    ...
    private static long makeLong(DataInput in, int bytes_to_read) throws IOException {
        long retval = 0;
        for (int i = 0; i < bytes_to_read; i++) {
            byte b = in.readByte();
            retval |= ((long) b & 0xff) << (i * 8);
    ...
    static void writeFloat(float num, byte[] buf, int offset) {
    ...
    static void writeDouble(double num, byte[] buf, int offset) {
    ...
    /**
     * Writes a string to buf. The length of the string is written first, followed by the chars (as
     * single-byte values). Multi-byte values are truncated: only the lower byte of each multi-byte
     * char is written, similar to {@link DataOutput#writeChars(String)}.
     *
     * @param s the string
     * @param buf the buffer
     */
    ...
    /**
     * Reads a string from buf. The length is read first, followed by the chars. Each char is a
     * single byte
     *
     * @param buf the buffer
    ...
    /**
     * Measures the number of bytes required to encode a string, taking multibyte characters into
     * account. Measures strings written by {@link DataOutput#writeUTF(String)}.
     *
    ...
    /**
     * Writes an AsciiString to buf. The length of the string is written first, followed by the
     * chars (as single-byte values).
     *
     * @param s the string
     * @param buf the buffer
     */
    ...
    /**
     * Reads an AsciiString from buf. The length is read first, followed by the chars. Each char is
     * a single byte
     *
     * @param buf the buffer
     * @return the string read from buf
    ...
    /**
     * Encodes the number of bytes needed into a single byte. The first number is encoded in the
     * first nibble (the first 4 bits), the second number in the second nibble
     *
     ...
     */
    private static byte encodeLength(byte len1, byte len2) {
        byte retval = len2;
        retval |= (len1 << 4);
        return retval;
    }
...
/*    protected static byte[] decodeLength(byte len) {
    ...
    private static byte firstNibble(byte len) {
        return (byte) ((len & 0xff) >> 4); // 0xf0 is the first nibble set (11110000)
    }

    private static byte secondNibble(byte len) {
        return (byte) (len & ~0xf0); // 0xf0 is the first nibble set (11110000)
    }

    private static byte bytesRequiredFor(long number) {
        if (number >> 56 != 0)
            return 8;
        if (number >> 48 != 0)
            return 7;
    ...
    private static byte bytesRequiredFor(int number) {
        if (number >> 24 != 0)
            return 4;
        if (number >> 16 != 0)
            return 3;
    ...
    private static byte getByteAt(long num, int index) {
