...
public class RspList<T> extends HashMap<Address, Rsp<T>> implements Iterable<Rsp<T>> {
    ...
    /**
     * Returns the first value in the response set. This is random, but we try to return a non-null
     * value first
     */
    public T getFirst() {
        Optional<Rsp<T>> retval = values().stream()
            .filter(rsp -> rsp.getValue() != null).findFirst();
    ...
    public List<T> getResults() {
        return values().stream().filter(rsp -> rsp.wasReceived() && rsp.getValue() != null)
            .collect(() -> new ArrayList<>(size()), (list, rsp) ->
                list.add(rsp.getValue()), (l, r) -> {
            });
    }

    public List<Address> getSuspectedMembers() {
        return entrySet().stream().filter(entry -> entry.getValue() != null
            && entry.getValue().wasSuspected()).map(Entry::getKey).collect(Collectors.toList());
    }
