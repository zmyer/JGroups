...
/**
 * Implementation of {@link TimeScheduler}. Uses a {@link DelayQueue} to order tasks according to
 * execution times
 *
 * @author Bela Ban
 * @since 3.3
...
public class TimeScheduler3 implements TimeScheduler, Runnable {
    ...
    /**
     * Thread which removes tasks ready to be executed from the queue and submits them to the pool
     * for execution
     */
    ...
    public TimeScheduler3(ThreadFactory factory, int min_threads, int max_threads,
        long keep_alive_time, int max_queue_size,
    ...
    public Future<?> scheduleWithFixedDelay(Runnable work, long initial_delay, long delay,
        TimeUnit unit, boolean can_block) {
        return scheduleRecurring(work, TaskType.fixed_delay, initial_delay, delay, unit, can_block);
    }

    public Future<?> scheduleAtFixedRate(Runnable work, long initial_delay, long delay,
        TimeUnit unit, boolean can_block) {
    ...
    protected Future<?> scheduleRecurring(Runnable work, TaskType type, long initial_delay,
        long delay, TimeUnit unit, boolean can_block) {
    ...
    protected class RecurringTask extends Task {
        ...
        public RecurringTask(Runnable runnable, TaskType type, long initial_delay, long delay,
            TimeUnit unit, boolean can_block) {
