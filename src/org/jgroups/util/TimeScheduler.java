...
/**
 * Timer-like interface which allows for execution of tasks. Taks can be executed <ul> <li>one time
 * only <li>at recurring time intervals. Intervals can be fixed-delay or fixed-rate, see {@link
 * java.util.concurrent.ScheduledExecutorService} for details <li>dynamic; at the end of the task
 * execution, a task is asked what the next execution time should be. To do this, method {@link
 * org.jgroups.util.TimeScheduler.Task#nextInterval()} needs to be implemented. </ul>
 *
 * @author Bela Ban
 */
...
public interface TimeScheduler {
    ...
    /**
     * The interface that dynamic tasks ({@link TimeScheduler#scheduleWithDynamicInterval(org.jgroups.util.TimeScheduler.Task)})
     * must implement
     */
     ...
     * <tt>schedule(command, 0, anyUnit)</tt>.
     *
     * @param command the task to execute
     * @throws java.util.concurrent.RejectedExecutionException at discretion of
     * <tt>RejectedExecutionHandler</tt>, if task cannot be accepted for execution because the
     ...
     */
    default Future<?> scheduleWithFixedDelay(Runnable command, long initialDelay, long delay,
        TimeUnit unit) {
     ...
     */
    default Future<?> scheduleAtFixedRate(Runnable command, long initialDelay, long period,
        TimeUnit unit) {
     ...
     * This is delay-based and not rate-based. The task is never done until nextInterval() return a
     * value <= 0 or the task is cancelled.
     *
    ...
    /**
     * Returns a list of tasks currently waiting for execution. If there are a lot of tasks, the
     * returned string should probably only return the number of tasks rather than a full dump.
     *
     * @return
     */
