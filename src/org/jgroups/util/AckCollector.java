...
public class AckCollector {
    ...
    private final List<Address> missing_acks;
    ...
    public synchronized AckCollector reset(Collection<Address> expected_acks,
    ...
    public String toString() {
        return suspected_mbrs.isEmpty() ?
            printMissing() : printMissing() + " (suspected: " + printSuspected() + ")";
    }
    ...
    private synchronized String printSuspected() {
    ...
    protected synchronized void addAll(Collection<Address> members, Collection<Address> exclude) {
        ...
        members.stream()
            .filter(member -> member != null
                && !missing_acks.contains(member)
                && (exclude != null && !exclude.contains(member))).forEach(missing_acks::add);
        expected_acks = missing_acks.size();
    }
    ...
    protected synchronized void addAll(Collection<Address> members, Address... exclude) {
        ...
        members.stream()
            .filter(member -> member != null
                && !missing_acks.contains(member)
                && (exclude != null && !Util.contains(member, exclude))).forEach(missing_acks::add);
        expected_acks = missing_acks.size();
    }
}
