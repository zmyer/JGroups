...
/**
 * Represents a message batch; multiple messages from the same sender to the same receiver(s). This
 * class is unsynchronized.
 *
 * @author Bela Ban
 * @since 3.3
 */
...
public class MessageBatch implements Iterable<Message> {
    ...
    /**
     * The destination address. Null if this is a multicast message batch, non-null if the batch is
     * sent to a specific member
     */
    ...
    /**
     * The name of the cluster in which the message batch is sent, this is equivalent to
     * TpHeader.cluster_name
     */
    ...
    private static final int INCR = 5; // number of elements to add when resizing
    private static final ToIntBiFunction<Message, MessageBatch> length_visitor =
        (msg, batch) -> msg != null ? msg.getLength() : 0;
    private static final ToLongBiFunction<Message, MessageBatch> total_size_visitor =
        (msg, batch) -> msg != null ? msg.size() : 0;
     ...
     * @return the number of messages from the other batch that were added successfully. Will always
     * be batch.size() unless resize==0: in this case, the number of messages that were added
     ...
     *
     * @param existing_msg The message to be replaced. The message has to be non-null and is found
     ...
     *
     * @param filter the filter. If null, no changes take place. Note that filter needs to be able
     * to handle null msgs
     * @param replacement the replacement message. Can be null, which essentially removes all
    ...
    /**
     * Transfers messages from other to this batch. Optionally clears the other batch after the
     * transfer
     *
    ...
    /**
     * Returns the size of the message batch (by calling {@link org.jgroups.Message#size()} on all
     * messages)
     */
    ...
    /**
     * Returns the total number of bytes of the message batch (by calling {@link
     * org.jgroups.Message#getLength()} on all messages)
     */
    public int length() {
        int retval = 0;
        for (int i = 0; i < index; i++)
    ...
    public String toString() {
        ...
        if (sb.length() > 0)
            sb.append(", ");
        sb.append(size()).append(" messages [capacity=").append(messages.length).append("]");

        return sb.toString();
    }

    public String printHeaders() {
        StringBuilder sb = new StringBuilder().append("dest=").append(dest);
        if (sender != null)
            sb.append(", sender=").append(sender);
        sb.append("\n").append(size()).append(":\n");
        int count = 1;
    ...
    private class BatchIterator implements Iterator<Message> {
        int current_index = -1;
        final int saved_index; // index at creation time of the iterator
