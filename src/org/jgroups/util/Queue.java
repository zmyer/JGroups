...
public class Queue {
    ...
    /**
     * Removes 1 element from the head. If the queue is empty the operation will wait for timeout
     * ms. if no object is added during the timeout time, a Timout exception is thrown (bela Aug
     * 2009) Note that the semantics of remove(long timeout) are weird - the method waits until an
     * element has been added, but doesn't do so in a loop ! So if we have 10 threads waiting on an
     * empty queue, and 1 thread adds an element, all 10 threads will return (but only 1 will have
     * the element), therefore 9 will throw a TimeoutException ! If I change this to the 'correct'
     * semantics, however (e.g. the method removeWait() below), GMS.ViewHandler doesn't work
     * correctly anymore. I won't change this now, as Queue will get removed anyway in 3.0.
     *
...
//            }
     ...
     *
     * @param timeout how long in milli seconds will this operation wait for an object to be added
     * to the queue before it times out
     * @return the first object on the queue
     */
     ...
     * Entries may be added and removed, but when the end-of-entries marker is encountered, the
     * queue is marked as closed. This allows to flush pending messages before closing the queue.
     */
