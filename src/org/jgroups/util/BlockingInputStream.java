...
/**
 * Bounded input stream. A consumer reads bytes until the end of the stream is reached, or the input
 * stream is closed. The producer writes bytes to the tail and blocks if the capacity has been
 * reached (until the consumer reads more bytes).
 * <p/>
 * This class is for only 1 producer and 1 consumer; multiple producers/consumers will most likely
 * yield incorrect results !
 * <p/>
 * Note that the implementation of this class is optimized for reading and adding a few K at a time;
 * performance will be suboptimal if single bytes are added and read.
 *
 * @author Bela Ban
 * @since 2.12.2
 */
...
public class BlockingInputStream extends InputStream {
    ...
    /**
     * Appends bytes to the end of the stream. If the number of bytes to be written is greater than
     * the remaining capacity, write() will block until the bytes can be added, or the stream is
     * closed.
     * <p/>
     * This method will try to append partial buffers to the stream, e.g. if the remaining capacity
     * is 2K, but the length of the buffer is 5K, 2K will be written and then write() will block
     * until the remaining 3K can be added.
     *
        ...
        try {
            ...
            while (bytes_written < length && !closed) {
                ...
                if (remaining() > 0) {
                    ...
                    System.arraycopy(buf, offset + bytes_written,
                        this.buf, write_pos, bytes_to_write);
    ...
    /**
     * Closes the stream. Writes to a closed stream will fail, reads will successfully read the
     * bytes that are already in the buffer and then return -1 (EOF)
     *
     * @throws IOException io exception
     */
    ...
    /**
     * Moves the bytes between [read_pos and write_pos] read_pos bytes to the left, such that the
     * new read_pos is 0 and the write_pos is write_pos - read_pos. Lock must be held.
     */
    @GuardedBy("lock")
     ...
     */
    private static void sanityCheck(byte[] buf, int offset, int length) {
        if (buf == null)
