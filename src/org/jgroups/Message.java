 ...
 * side. <p> The byte buffer can point to a reference, and we can subset it using index and length.
...
public class Message implements Streamable, Constructable<Message> {
     ...
     *
     * @param dest The Address of the receiver. If it is null, then the message is sent to the
     * group. Otherwise, it is sent to a single member.
     */
     ...
     * rather than a copy, and refer to a subset of the buffer. This is important when we want to
     * avoid copying. When the message is serialized, only the subset is serialized.</p> <em> Note
     * that the byte[] buffer passed as argument must not be modified. Reason: if we retransmit the
     * message, it would still have a ref to the original byte[] buffer passed in as argument, and
     * so we would retransmit a changed byte[] buffer ! </em>
     *
     * @param dest The Address of the receiver. If it is null, then the message is sent to the
     * group. Otherwise, it is sent to a single member.
     ...
     * @param dest The Address of the receiver. If it is null, then the message is sent to the
     * group. Otherwise, it is sent to a single member.
     * @param obj The object that will be marshalled into the byte buffer. Has to be serializable
    ...
    /**
     * Returns a <em>reference</em> to the payload (byte buffer). Note that this buffer should not
     * be modified as we do not copy the buffer on copy() or clone(): the buffer of the copied
     * message is simply a reference to the old buffer.<br/> Even if offset and length are used: we
     * return the <em>entire</em> buffer, not a subset.
     */
    ...
    /**
     * Sets the buffer.<p/> Note that the byte[] buffer passed as argument must not be modified.
     * Reason: if we retransmit the message, it would still have a ref to the original byte[] buffer
    ...
    /**
     * Sets the internal buffer to point to a subset of a given buffer.<p/> <em> Note that the
     * byte[] buffer passed as argument must not be modified. Reason: if we retransmit the message,
     * it would still have a ref to the original byte[] buffer passed in as argument, and so we
     * would retransmit a changed byte[] buffer ! </em>
     *
     ...
     * serialization and only pass byte[] buffer to JGroups.<p/> As of 3.5, a classloader can be
     * passed in. It will be used first to find a class, before contacting the other classloaders in
     * the list. If null, the default list of classloaders will be used.
     *
     * @return the object
     */
     ...
     * Create a copy of the message. If offset and length are used (to refer to another buffer), the
     * copy will contain only the subset offset and length point to, copying the subset into the new
     * copy.<p/> Note that for headers, only the arrays holding references to the headers are
     * copied, not the headers themselves ! The consequence is that the headers array of the copy
    ...
    /**
     * Reads the message's contents from an input stream, but skips the buffer and instead returns
     * the position (offset) at which the buffer starts
     */
    ...
    /**
     * Returns the exact size of the marshalled message. Uses method size() of each header to
     * compute the size, so if a Header subclass doesn't implement size() we will use an
     * approximation. However, most relevant header subclasses have size() implemented correctly.
     * (See org.jgroups.tests.SizeTest).<p/> The return type is a long as this is the length of the
     * payload ({@link #getLength()}) plus metadata (e.g. flags, headers, source and dest addresses
     * etc). Since the largest payload can be Integer.MAX_VALUE, adding the metadata might lead to
     * an int overflow, that's why we use a long.
     *
