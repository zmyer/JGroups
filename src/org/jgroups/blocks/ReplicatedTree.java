...
/**
 * A tree-like structure that is replicated across several members. Updates will be multicast to all
 * group members reliably and in the same order.
 *
...
public class ReplicatedTree extends ReceiverAdapter {
    ...
    private Node root = new Node(SEPARATOR, SEPARATOR, null, null);
    private final List<ReplicatedTreeListener> listeners = new ArrayList<>();
    JChannel channel = null;
    private String groupname = "ReplicatedTree-Group";
    final List<Address> members = new ArrayList<>();
    private long state_fetch_timeout = 10000;
     ...
     * Useful for testing
     */
    private boolean remote_calls = true;
    ...
    interface ReplicatedTreeListener {
    ...
    /**
     * Creates a channel with the given properties. Connects to the channel, then creates a
     * PullPushAdapter and starts it
     */
    public ReplicatedTree(String groupname, String props,
        long state_fetch_timeout) throws Exception {
        if (groupname != null)
        ...
        if (jmx) {
            ...
            if (server == null)
                throw new Exception("No MBeanServers found; " +
                    "need to run with an MBeanServer present, or inside JDK 5");
            JmxConfigurator.registerChannel(channel, server,
                "jgroups", channel.getClusterName(), true);
        }
        start();
    }
    ...
    /**
     * Fetch the group state from the current coordinator. If successful, this will trigger
     * setState().
     */
    ...
    private void addReplicatedTreeListener(ReplicatedTreeListener listener) {
    ...
    /**
     * Adds a new node to the tree and sets its data. If the node doesn not yet exist, it will be
     * created. Also, parent nodes will be created if not existent. If the node already has data,
     * then the new data will override the old one. If the node already existed, a nodeModified()
     * notification will be generated. Otherwise a nodeCreated() motification will be emitted.
     *
        ...
        if (send_message) {
            if (channel == null) {
                if (log.isErrorEnabled())
                    log.error("channel is null, cannot broadcast PUT request");
                return;
            }
            try {
    ...
    /**
     * Adds a key and value to a given node. If the node doesn't exist, it will be created. If the
     * node already existed, a nodeModified() notification will be generated. Otherwise a
    ...
    public void remove(String fqn) {
        ...
        if (send_message) {
            if (channel == null) {
                if (log.isErrorEnabled())
                    log.error("channel is null, cannot broadcast REMOVE request");
                return;
            }
            try {
    ...
    /**
     * Finds a node given its name and returns the value associated with a given key in its {@code
     * data} map. Returns null if the node was not found in the tree or the key was not found in the
     * hashmap.
     *
     * @param fqn The fully qualified name of the node.
     * @param key The key.
    ...
    /**
     * Returns the data hashmap for a given node. This method can only be used by callers that are
     * inside the same package. The reason is that callers must not modify the return value, as
     * these modifications would not be replicated, thus rendering the replicas inconsistent.
     *
     * @param fqn The fully qualified name of the node
    ...
    private void _put(String fqn, HashMap data) {
        Node n;
    ...
    private void _put(String fqn, String key, Object value) {
        Node n;
        StringHolder child_name = new StringHolder();
        boolean child_exists;
    ...
    private void _remove(String fqn, String key) {
        Node n;
    ...
    public void receive(Message msg) {
        ...
        try {
            ...
            switch (req.type) {
                ...
                default:
                    if (log.isErrorEnabled())
                        log.error("type " + req.type + " unknown");
                    break;
            }
        } catch (Exception ex) {
            if (log.isErrorEnabled())
    ...
    /**
     * Find the node just <em>above</em> the one indicated by {@code fqn}. This is needed in many
     * cases, e.g. to add a new node or remove an existing node.
     *
     * @param fqn The fully qualified name of the node.
     * @param child_name Will be filled with the name of the child when this method returns. The
     * child name is the last relative name of the {@code fqn}, e.g. in "/a/b/c" it would be "c".
     * @param create_if_not_exists Create parent nodes along the way if they don't exist. Otherwise,
     * this method will return when a node cannot be found.
     */
    private Node findParentNode(String fqn, StringHolder child_name,
        ...
        if (fqn == null || fqn.equals(SEPARATOR) || fqn.isEmpty())
            return curr;
    ...
    /**
     * Returns the node at fqn. This method should not be used by clients (therefore it is
     * package-private): it is only used internally (for navigation). C++ 'friend' would come in
     * handy here...
     *
     * @param fqn The fully qualified name of the node
    ...
    private void notifyNodeAdded(String fqn) {
        for (ReplicatedTreeListener listener : listeners)
            listener.nodeAdded(fqn);
    }

    private void notifyNodeRemoved(String fqn) {
        for (ReplicatedTreeListener listener : listeners)
            listener.nodeRemoved(fqn);
    }

    private void notifyNodeModified(String fqn) {
        for (ReplicatedTreeListener listener : listeners)
            listener.nodeModified(fqn);
    }

    private void notifyViewChange(View v) {
        for (ReplicatedTreeListener listener : listeners)
            listener.viewChange(v);
    }

    /**
     * Generates NodeAdded notifications for all nodes of the tree. This is called whenever the tree
     * is initially retrieved (state transfer)
     */
    ...
    public static final class Node implements Serializable {
        ...
        private Node(String child_name, String fqn, Node parent, Map<String, Object> data) {
            ...
            this.parent = parent;
            if (data != null)
                this.data = (HashMap<String, Object>) ((HashMap) data).clone();
        }
        ...
        void print(StringBuilder sb, int indent) {
            ...
            if (children != null && !children.isEmpty()) {
                ...
                for (Object value : values) {
                    sb.append('\n');
                    ((Node) value).print(sb, indent + INDENT);
                }
            }
        }
        ...
        public Object clone() throws CloneNotSupportedException {
            ...
            if (children != null)
                n.children = (TreeMap) children.clone();
            return n;
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            tree = new ReplicatedTree(null, null, 10000);
    ...
    private static class MyListener implements ReplicatedTreeListener {
