...
public class RequestOptions {
    ...
    /**
     * The max time (in ms) for a blocking call. 0 blocks until all responses have been received (if
     * mode = GET_ALL)
     */
    protected long timeout; // used when mode != GET_NONE

    /**
     * Turns on anycasting; this results in multiple unicasts rather than a multicast for group
     * calls
     */
    private boolean use_anycasting;

    /**
     * If use_anycasting is true: do we want to use an AnycastAddress [B,C] or a unicast to B and
     * another unicast to C to send an anycast to {B,C} ? Only used if use_anycasting is true
     */
    private boolean use_anycast_addresses;
    ...
    RequestOptions rspFilter(RspFilter filter) {
        this.rsp_filter = filter;
        return this;
    }
    ...
    private boolean flagSet(Message.Flag flag) {
        return flag != null && ((flags & flag.value()) == flag.value());
    }

    boolean transientFlagSet(Message.TransientFlag flag) {
    ...
    /**
     * Not recommended as the internal representation of flags might change (e.g. from short to
     * int). Use {@link #setFlags(Message.Flag...)} instead
     */
    ...
    private RequestOptions setTransientFlags(Message.TransientFlag... flags) {
    ...
    /**
     * Not recommended as the internal representation of flags might change (e.g. from short to
     * int). Use {@link #setTransientFlags(Message.TransientFlag...)} instead.
     */
    ...
    public String toString() {
        ...
        sb.append("mode=").append(mode).append(", timeout=").append(timeout);
        if (use_anycasting) {
        ...
        if (flags > 0)
            sb.append(", flags=").append(Message.flagsToString(flags));
        if (transient_flags > 0)
            sb.append(", transient_flags=").append(Message.transientFlagsToString(transient_flags));
        if (exclusion_list != null)
            sb.append(", exclusion list: ").append(Arrays.toString(exclusion_list));
        return sb.toString();
    }
}
