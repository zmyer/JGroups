...
class UnicastRequest<T> extends Request<T> {
    ...
    public void sendRequest(Buffer data) throws Exception {
        try {
            corr.sendUnicastRequest(target, data,
                options.mode() == ResponseMode.GET_NONE ? null : this, this.options);
        } catch (Exception ex) {
            corrDone();
            throw ex;
        }
    ...
    public void siteUnreachable(String site) {
        if (!(target instanceof SiteAddress)
            || !((SiteAddress) target).getSite().equals(site) || isDone())
            return;
    ...
    /**
     * If the target address is not a member of the new view, we'll mark the response as suspected
     * and unblock the caller of execute()
     */
