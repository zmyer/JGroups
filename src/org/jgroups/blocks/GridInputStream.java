...
@Experimental class GridInputStream extends InputStream {
    ...
    private byte[] current_buffer = null;
    private boolean end_reached = false;
    final static Log log = LogFactory.getLog(GridInputStream.class);

    GridInputStream(GridFile file, ReplCache<String, byte[]> cache,
        int chunk_size) throws FileNotFoundException {
        this.file = file;
    ...
    private byte[] fetchNextChunk() {
        ...
        if (log.isTraceEnabled())
            log.trace("fetching index=" + index
                + ", key=" + key + ": " + (val != null ? val.length + " bytes" : "null"));
        return val;
    }
