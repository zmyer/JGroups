 ...
 * Multiple requests can be sent at a time. Whenever a response is received, the correct {@code
 * Request} is looked up (key = id) and its method {@code receiveResponse()} invoked.
 *
 * @author Bela Ban
 */
...
public class RequestCorrelator {
     ...
     * Can be either a Protocol or a Transport
     */
    ...
    private short corr_id = ClassConfigurator.getProtocolId(this.getClass());
    ...
    private boolean async_dispatching;

    private boolean wrap_exceptions = false;

    private final MyProbeHandler probe_handler = new MyProbeHandler();

    private final RpcStats rpc_stats = new RpcStats(false);

    protected static final Log log = LogFactory.getLog(RequestCorrelator.class);

    /**
     * Constructor. Uses transport to send messages. If {@code handler} is not null, all incoming
     * requests will be dispatched to it (via {@code handle(Message)}).
     *
     * @param corr_id Used to differentiate between different RequestCorrelators (e.g. in different
     * protocol layers). Has to be unique if multiple request correlators are used.
     * @param transport Used to send/pass up requests.
     * @param handler Request handler. Method {@code handle(Message)} will be called when a request
     * is received.
     */
    public RequestCorrelator(short corr_id, Protocol transport, RequestHandler handler,
        Address local_addr) {
    ...
    RequestCorrelator asyncDispatching(boolean flag) {
    ...
    RequestCorrelator wrapExceptions(boolean flag) {
        wrap_exceptions = flag;
        return this;
    }

    /**
     * Sends a request to a group. If no response collector is given, no responses are expected
     * (making the call asynchronous)
     *
     * @param dest_mbrs The list of members who should receive the call. Usually a group RPC is sent
     * via multicast, but a receiver drops the request if its own address is not in this list. Will
     * not be used if it is null.
     * @param data the data to be sent.
     * @param req A request (usually the object that invokes this method). Its methods {@code
     * receiveResponse()} and {@code suspect()} will be invoked when a message has been received or
     * a member is suspected.
     */
    public void sendRequest(Collection<Address> dest_mbrs, Buffer data, Request req,
        ...
        Header hdr = opts.hasExclusionList() ?
            new MultiDestinationHeader(Header.REQ, 0, this.corr_id, opts.exclusionList())
            ...
            if (opts.useAnycastAddresses()) {
                ...
                for (Address mbr : dest_mbrs) {
                    ...
                    if (!mbr.equals(local_addr)
                        && copy.isTransientFlagSet(Message.TransientFlag.DONT_LOOPBACK))
    ...
    public void sendUnicastRequest(Address dest, Buffer data, Request req,
        RequestOptions opts) throws Exception {
        if (transport == null) {
    ...
    /**
     * Used to signal that a certain request may be garbage collected as all responses have been
     * received.
     */
    ...
    /**
     * <b>Callback</b>. <p> Called by the protocol below when a message has been received. The
     * algorithm should test whether the message is destined for us and, if not, pass it up to the
     * next layer. Otherwise, it should remove the header and check whether the message is a request
     * or response. In the first case, the message will be delivered to the request handler
     * registered (calling its {@code handle()} method), in the second case, the corresponding
     * response collector is looked up and the message delivered.
     *
     * @param evt The event to be received
     * @return Whether or not the event was consumed. If true, don't pass message up, else pass it
     * up
     */
    ...
    /**
     * An entire site is down; mark all requests that point to that site as unreachable (used by
     * RELAY2)
     */
    private void setSiteUnreachable(String site) {
        requests.values().stream().filter(Objects::nonNull)
            .forEach(req -> req.siteUnreachable(site));
    }

    /**
     * View received: mark all responses from members that are not in new_view as suspected
     */
    private void receiveView(View new_view) {
        view = new_view; // move this before the iteration (JGRP-1428)
        requests.values().stream().filter(Objects::nonNull)
            .forEach(req -> req.viewChange(new_view));
    }
    ...
    boolean receiveMessage(Message msg) {
        ...
        if (hdr instanceof MultiDestinationHeader) {
            ...
            if (exclusion_list != null && local_addr != null
                && Util.contains(local_addr, exclusion_list)) {
    ...
    void receiveMessageBatch(MessageBatch batch) {
    ...
    private RequestCorrelator removeEntry(long id) {
        ...
        if (req != null) {
            ...
            if (req instanceof UnicastRequest)
                ...
                log.error("request type %s not known", req.getClass().getSimpleName());
        }
        return this;
    }

    private void dispatch(final Message msg, final Header hdr) {
        switch (hdr.type) {
            case Header.REQ:
            ...
            case Header.EXC_RSP:
                ...
                if (req != null)
                    handleResponse(req, msg.src(), msg.getRawBuffer(),
                        msg.getOffset(), msg.getLength(), hdr.type == Header.EXC_RSP);
                break;
    ...
    private void handleRequest(Message req, Header hdr) {
        ...
        if (log.isTraceEnabled())
            log.trace("calling (%s) with request %d",
                request_handler != null ? request_handler.getClass().getName()
                    : "null", hdr.req_id);
    ...
    private void handleResponse(Request req, Address sender, byte[] buf, int offset, int length,
    ...
    private void sendReply(final Message req, final long req_id, Object reply,
        boolean is_exception) {
        Buffer rsp_buf;
    ...
    private void sendResponse(Message rsp, long req_id, boolean is_exception) {
    ...
    private static Buffer replyToBuffer(Object obj, Marshaller marshaller) throws Exception {
    ...
    private static Object replyFromBuffer(final byte[] buf, int offset, int length,
    ...
    public static class Header extends org.jgroups.Header {
        ...
        static final byte EXC_RSP = 2; // exception
         ...
         * @param req_id id of this header relative to ids of other requests originating from the
         * same correlator
        ...
        public String toString() {
            ...
            ret.append("corr_id=").append(corrId).append(", type=");
            switch (type) {
                case REQ:
                    ret.append("REQ");
                    break;
            ...
            ret.append(", req_id=").append(req_id).append(", rsp_expected=").append(rspExpected());
            return ret.toString();
        }
    ...
    public static final class MultiDestinationHeader extends Header {
        /**
         * Contains a list of members who should not receive the request (others will drop). Ignored
         * if null
         */
        ...
        public MultiDestinationHeader(byte type, long id,
            short corr_id, Address[] exclusion_list) {
    ...
    // TODO: 17/7/4 by zmyer
    private class MyProbeHandler implements DiagnosticsHandler.ProbeHandler {
