...
public class NioClient extends NioBaseServer implements Client {
    ...
    /**
     * Creates an instance of an {@link NioClient} that acts as a <em>client</em>: no server channel
     * is created and no acceptor is started to listen for incoming connections. Instead, a channel
     * is created (bound to bind_addr/bind_port) and connected to server_addr/server_port. This is
     * used to send messages to the remote server and receive messages from it. Note that there is
     * only a single TCP connection established between the client and server.
     *
     * @param bind_addr The address to which the local channel should bind to. Can be null, then the
     ...
     */
    public NioClient(InetAddress bind_addr, int bind_port, InetAddress server_addr,
        int server_port) {
    ...
    public void start() throws Exception {
        if (running.compareAndSet(false, true)) {
            ...
            acceptor = factory.newThread(new Acceptor(),
                "NioClient.Acceptor [srv=" + remote_addr + "]");
