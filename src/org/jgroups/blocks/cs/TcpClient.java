...
public class TcpClient extends TcpBaseServer implements Client, ConnectionListener {
    ...
    /**
     * Creates an instance of an {@link TcpClient} that acts as a <em>client</em>: no server socket
     * is created and no acceptor is started to listen for incoming connections. Instead, a client
     * socket is created (bound to bind_addr/bind_port) and connected to server_addr/server_port.
     * This is used to send messages to the remote server and receive messages from it. Note that
     * there is only a single TCP connection established between the client and server.
     *
