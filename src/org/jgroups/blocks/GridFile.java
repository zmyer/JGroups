...
public class GridFile extends File {
    ...
    GridFile(String pathname, ReplCache<String, Metadata> cache, int chunk_size,
        GridFilesystem fs) {
        super(pathname);
        this.fs = fs;
    ...
    GridFile(String parent, String child, ReplCache<String, Metadata> cache, int chunk_size,
        GridFilesystem fs) {
        super(parent, child);
        this.fs = fs;
    ...
    GridFile(File parent, String child, ReplCache<String, Metadata> cache, int chunk_size,
        GridFilesystem fs) {
        super(parent, child);
        this.fs = fs;
    ...
    public String getPath() {
        ...
        if (my_path.endsWith(File.separator)) {
    ...
    public boolean createNewFile() throws IOException {
        ...
        if (!checkParentDirs(getPath(), false))
            return false;
        cache.put(getPath(),
            new Metadata(0, System.currentTimeMillis(),
                chunk_size, Metadata.FILE), (short) -1, 0, true);
        return true;
    }
    ...
    public boolean mkdir() {
        try {
            ...
            if (!parents_exist)
                return false;
            cache.put(getPath(), new Metadata(0, System.currentTimeMillis(),
                chunk_size, Metadata.DIR), (short) -1, 0, true);
            return true;
        } catch (IOException e) {
            e.printStackTrace();
            return false;
    ...
    private File[] _listFiles(Object filter) {
    ...
    private String[] _list(Object filter) {
        ...
        for (String str : keys) {
            if (isChildOf(getPath(), str)) {
                if (filter instanceof FilenameFilter
                    && !((FilenameFilter) filter).accept(new File(name), filename(str)))
                    continue;
                else if (filter instanceof FileFilter
                    && !((FileFilter) filter).accept(new File(str)))
                    continue;
                list.add(str);
            }
        }
    ...
    /**
     * Checks whether the parent directories are present (and are directories). If create_if_absent
     * is true, creates missing dirs
     *
     * @param path path
     * @param create_if_absent create if absent
     */
    private boolean checkParentDirs(String path, boolean create_if_absent) throws IOException {
        ...
        for (int i = 0; i < components.length - 1; i++) {
            ...
            if (exists(comp)) {
                ...
                if (create_if_absent)
                    cache.put(comp, new Metadata(0, System.currentTimeMillis(),
                        chunk_size, Metadata.DIR), (short) -1, 0);
                else
                    return false;
            }
        }
    ...
    public static class Metadata implements Streamable {
        ...
        Metadata(int length, long modification_time, int chunk_size, byte flags) {
            this.length = length;
        ...
        public String toString() {
            ...
            sb.append(getType());
            if (is_file)
                sb.append(", len=").append(Util.printBytes(length))
                    .append(", chunk_size=").append(chunk_size);
            sb.append(", mod_time=").append(new Date(modification_time));
            return sb.toString();
        }
