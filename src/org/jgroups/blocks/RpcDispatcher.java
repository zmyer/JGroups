...
/**
 * This class allows a programmer to invoke remote methods in all (or single) group members and
 * optionally wait for the return value(s).<p/> An application will typically create a channel and
 * layer the RpcDispatcher building block on top of it, which allows it to dispatch remote methods
 * (client role) and at the same time be called by other members (server role).<p/> This class is
 * derived from MessageDispatcher.
 *
 * @author Bela Ban
 */
...
public class RpcDispatcher extends MessageDispatcher {
    ...
    /**
     * Marshaller to marshall requests at the caller, unmarshal requests at the receiver(s),
     * marshall responses at the receivers and unmarshall responses at the caller
     */
    protected Marshaller marshaller;

    private MethodLookup method_lookup;
    ...
    RpcDispatcher setServerObject(Object server_obj) {
        this.server_obj = server_obj;
        return this;
    }
     ...
     * @return RspList<T> A response list with results, one for each member in dests, or null if the
     * RPC is asynchronous
     * @throws Exception If the sending of the message threw an exception. Note that <em>no</em>
     * exception will be thrown if any of the target members threw an exception, but this exception
     * will be in the Rsp object for the particular member in the RspList
     */
    public <T> RspList<T> callRemoteMethods(Collection<Address> dests, String method_name,
        Object[] args,
     ...
     * @return RspList A list of return values and flags (suspected, not received) per member, or
     * null if the RPC is asynchronous
     * @throws Exception If the sending of the message threw an exception. Note that <em>no</em>
     * exception will be thrown if any of the target members threw an exception, but this exception
     * will be in the Rsp object for the particular member in the RspList
     * @since 2.9
     ...
     * @param method_call The method (plus args) to be invoked
     * @param options A collection of call options, e.g. sync versus async, timeout etc
     * @return CompletableFuture A future from which the results can be fetched, or null if the RPC
     * is asynchronous
     * @throws Exception If the sending of the message threw an exception. Note that <em>no</em>
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> CompletableFuture<RspList<T>> callRemoteMethodsWithFuture(Collection<Address> dests,
     ...
     * @return The result. Null if the call is asynchronous (non-blocking) or if the method returns
     * void
     * @throws Exception Thrown if the method invocation threw an exception, either at the caller or
     * the callee
     */
    @SuppressWarnings("unchecked")
    public <T> T callRemoteMethod(Address dest, String meth, Object[] args, Class[] types,
        RequestOptions opts) throws Exception {
    ...
    public <T> T callRemoteMethod(Address dest, MethodCall call,
        ...
        if (log.isTraceEnabled())
            log.trace("dest=%s, method_call=%s, options=%s, retval: %s",
                dest, call, options, retval);
        return retval;
    }
     ...
     * ExecutionException will be thrown on calling Future.get(). If the invocation was
     * asynchronous, null will be returned.
     * @throws Exception Thrown if the method invocation threw an exception
     */
    public <T> CompletableFuture<T> callRemoteMethodWithFuture(Address dest, MethodCall call,
        RequestOptions opts) throws Exception {
        if (log.isTraceEnabled())
    ...
    public Object handle(Message req) throws Exception {
        ...
        MethodCall method_call = methodCallFromBuffer(req.getRawBuffer(),
            req.getOffset(), req.getLength(), marshaller);
        if (log.isTraceEnabled())
    ...
    private static Buffer methodCallToBuffer(final MethodCall call,
        ...
        if (args != null)
            for (Object arg : args)
                estimated_size +=
                    marshaller != null ? marshaller.estimatedSize(arg) : (arg == null ? 2 : 50);
    ...
    private static MethodCall methodCallFromBuffer(final byte[] buf, int offset, int length,
        Marshaller marshaller) throws Exception {
