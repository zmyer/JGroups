...
/**
 * Simple cache which maintains keys and value. A reaper can be enabled which periodically evicts
 * expired entries. Also, when the cache is configured to be bounded, entries in excess of the max
 * size will be evicted on put().
 *
 * @author Bela Ban
 */
...
public class Cache<K, V> {
    ...
    /**
     * The maximum number of keys, When this value is exceeded we evict older entries, until we drop
     * below this mark again. This effectively maintains a bounded cache. A value of 0 means don't
     * bound the cache.
     */
     ...
     * @param key key
     * @param val value
     * @param caching_time Number of milliseconds to keep an entry in the cache. -1 means don't
     * cache (if reaping is enabled, we'll evict an entry with -1 caching time), 0 means never
     * evict. In the latter case, we can still evict an entry with 0 caching time: when we have a
     * bounded cache, we evict in order of insertion no matter what the caching time is.
     */
    @ManagedOperation
    ...
    /**
     * This method should not be used to add or remove elements ! It was just added because
     * ReplCacheDemo requires it for its data model
     */
    public ConcurrentMap<K, Value<V>> getInternalMap() {
        return map;
    }

    Value<V> getEntry(K key) {
        if (log.isTraceEnabled())
        ...
        for (Map.Entry<K, Value<V>> entry : map.entrySet()) {
            ...
            if (val != null) {
                if (val instanceof byte[])
                    sb.append(" (").append(((byte[]) val).length).append(" bytes)");
                else
                    sb.append(val);
            }
            sb.append("\n");
