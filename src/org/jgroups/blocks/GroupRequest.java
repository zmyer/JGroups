 ...
 * boolean value (success or failure). Results (if any) can be retrieved when done. <p> The
 * supported transport to send requests is currently either a RequestCorrelator or a generic
 * Transport. One of them has to be given in the constructor. It will then be used to send a
 * request. When a message is received by either one, the receiveResponse() of this class has to be
 * called (this class does not actively receive requests/responses itself). Also, when a view change
 * or suspicion is received, the methods viewChange() or suspect() of this class have to be called.
 * <p> When started, an array of responses, correlating to the membership, is created. Each response
 * is added to the corresponding field in the array. When all fields have been set, the algorithm
 * terminates. This algorithm can optionally use a suspicion service (failure detector) to detect
 * (and exclude from the membership) fauly members. If no suspicion service is available, timeouts
 * can be used instead (see {@code execute()}). When done, a list of suspected members can be
 * retrieved. <p> Because a channel might deliver requests, and responses to <em>different</em>
 * requests, the {@code GroupRequest} class cannot itself receive and process requests/responses
 * from the channel. A mechanism outside this class has to do this; it has to determine what the
 * responses are for the message sent by the {@code execute()} method and call {@code
...
public class GroupRequest<T> extends Request<RspList<T>> {
    ...
    @GuardedBy("lock")
    private int num_valid;       // valid responses (values or exceptions that passed the response filter)

    @GuardedBy("lock")
    private int num_received;    // number of responses (values, exceptions or suspicions)

    /**
     * @param corr The request correlator to be used. A request correlator sends requests tagged
     * with a unique ID and notifies the sender when matching responses are received. The reason
     * {@code GroupRequest} uses it instead of a {@code Transport} is that multiple
     * requests/responses might be sent/received concurrently
     * @param targets The targets, which are supposed to receive the message. Any receiver not in
     * this set will discard the message. Targets are always a subset of the current membership
     * @param options The request options to be used for this call
     */
    public GroupRequest(RequestCorrelator corr, Collection<Address> targets,
     ...
     * table. When all responses have been received, {@code execute()} returns.
     */
        ...
        try {
            for (Map.Entry<Address, Rsp<T>> entry : rsps.entrySet()) {
                ...
                if (rsp != null && !(rsp.wasReceived() ||
                    rsp.wasSuspected() || rsp.wasUnreachable())) {
    ...
    private RspList<T> doAndComplete(Callable<RspList<T>> supplier) {
        try {
    ...
    protected void sendRequest(Buffer data,
        final Collection<Address> targetMembers) throws Exception {
        try {
            corr.sendRequest(targetMembers, data,
                options.mode() == ResponseMode.GET_NONE ? null : this, options);
        } catch (Exception ex) {
            corrDone();
            throw ex;
        }
