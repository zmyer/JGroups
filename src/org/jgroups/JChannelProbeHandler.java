...
class JChannelProbeHandler implements DiagnosticsHandler.ProbeHandler {
    ...
    public String[] supportedKeys() {
        return new String[] {
            "reset-stats", "jmx", "op=<operation>[<args>]",
            "threads[=<filter>[=<limit>]]", "enable-cpu",
            "enable-contention", "disable-cpu", "disable-contention"};
    }
    ...
    protected static String enable(int type, boolean flag) {
        ...
        return String.format("%s monitoring supported: %b, %s monitoring enabled: %b",
            tmp, supported, tmp, supported && flag);
    }

    private JChannel resetAllStats() {
    ...
    private void handleJmx(Map<String, String> map, String input) {
                    ...
                    if (index != -1) {
                        ...
                        if (field != null) {
                            ...
                            ResourceDMBean.Accessor setter =
                                ResourceDMBean.findSetter(target, attrname);
                            if (setter == null && target instanceof AdditionalJmxObjects) {
                                Object[] objs = ((AdditionalJmxObjects) target).getJmxObjects();
                                if (objs != null && objs.length > 0) {
                                    for (Object o : objs) {
                                        setter = o != null ?
                                            ResourceDMBean.findSetter(target, attrname) : null;
                                        if (setter != null)
                                            break;
                                    }
                                }
                            ...
                            if (setter != null) {
                                try {
                                    ...
                                    log.error("unable to invoke %s() on %s: %s",
                                        setter, protocol_name, e);
                                }
                            } else
    ...
    private void handleOperation(Map<String, String> map, String operation) throws Exception {
        ...
        if (method == null) {
            ...
            if (method == null) {
                log.warn(Util.getMessage("MethodNotFound"),
                    ch.getAddress(), target.getClass().getSimpleName(), method_name);
                return;
            }
        }
    ...
    private static class ThreadEntry {
        ...
        final double block_time, wait_time;  // ms
        final double cpu_time, user_time;    // ms

        ThreadEntry(Thread.State state, String thread_name, long blocks, long waits,
            double block_time, double wait_time,
        ...
        public String toString() {
            return String.format("[%s] %s:", state, thread_name) + String.format(" blocks=%d (%.2f ms) waits=%d (%.2f ms)", blocks, block_time, waits, wait_time) +
                String.format(" sys=%.2f ms user=%.2f ms\n", cpu_time, user_time);
        }
