...
public abstract class Protocol {
    ...
    private String after_creation_hook;
     ...
     *
     * @param level The new level. Valid values are "fatal", "error", "warn", "info", "debug",
     * "trace" (capitalization not relevant)
     */
    ...
    /**
     * After configuring the protocol itself from the properties defined in the XML config, a
     * protocol might have additional objects which need to be configured. This callback allows a
     * protocol developer to configure those other objects. This call is guaranteed to be invoked
    ...
    /**
     * Called by the XML parser when subelements are found in the configuration of a protocol. This
    ...
    /**
     * Supposed to be overwritten by subclasses. Usually the transport returns a valid non-null
     * thread factory, but thread factories can also be created by individual protocols
     */
    ...
    /**
     * Returns the SocketFactory associated with this protocol, if overridden in a subclass, or
     ...
     * sent.
     *
     * @throws Exception Thrown if protocol cannot be initialized successfully. This will cause the
     ...
     *
     * @throws Exception Thrown if protocol cannot be started successfully. This will cause the
     * ProtocolStack to fail, so {@link JChannel#connect(String)} will throw an exception
     */
    public void start() throws Exception {
    }

    /**
     * This method is called on a {@link JChannel#disconnect()}. Stops work (e.g. by closing
     * multicast socket). Will be called <em>from top to bottom</em>. This means that at the time of
     * the method invocation the neighbor protocol below is still working. This method will replace
     * the <b>STOP</b>, <b>STOP_OK</b>, <b>CLEANUP</b> and <b>CLEANUP_OK</b> events. The
    ...
    List<Integer> requiredUpServices() {
        return null;
    }
    ...
    protected final List<Integer> getDownServices() {
    ...
    /**
     * An event is to be sent down the stack. A protocol may want to examine its type and perform
     * some action on it, depending on the event's type. If the event is a message MSG, then the
     * protocol may need to add a header to it (or do nothing at all) before sending it down the
     * stack using {@code down_prot.down()}.
     */
    ...
    /**
     * A message is sent down the stack. Protocols may examine the message and do something (e.g.
     * add a header) with it before passing it down.
     *
     * @since 4.0
     */
    ...
    /**
     * An event was received from the protocol below. Usually the current protocol will want to
     * examine the event type and - depending on its type - perform some computation (e.g. removing
     * headers from a MSG event type, or updating the internal membership list when receiving a
     * VIEW_CHANGE event). Finally the event is either a) discarded, or b) an event is sent down the
     * stack using {@code down_prot.down()} or c) the event (or another event) is sent up the stack
     * using {@code up_prot.up()}.
     */
    ...
    /**
     * A single message was received. Protocols may examine the message and do something (e.g. add a
     * header) with it before passing it up.
     *
     * @since 4.0
     */
    ...
    /**
     * Sends up a multiple messages in a {@link MessageBatch}. The sender of the batch is always the
     * same, and so is the destination (null == multicast messages). Messages in a batch can be OOB
     * messages, regular messages, or mixed messages, although the transport itself will create
     * initial MessageBatches that contain only either OOB or regular messages.<p/> The default
     * processing below sends messages up the stack individually, based on a matching criteria
     * (calling {@link #accept(org.jgroups.Message)}), and - if true - calls {@link
     * #up(org.jgroups.Event)} for that message and removes the message. If the batch is not empty,
     * it is passed up, or else it is dropped.<p/> Subclasses should check if there are any messages
     * destined for them (e.g. using {@link MessageBatch#getMatchingMessages(short, boolean)}), then
     * possibly remove and process them and finally pass the batch up to the next protocol.
     * Protocols can also modify messages in place, e.g. ENCRYPT could decrypt all encrypted
     * messages in the batch, not remove them, and pass the batch up when done.
     *
    ...
    /**
     * Called by the default implementation of {@link #up(org.jgroups.util.MessageBatch)} for each
     * message to determine if the message should be removed from the message batch (and handled by
     * the current protocol) or not.
     *
     * @param msg The message. Guaranteed to be non-null
     * @return True if the message should be handled by this protocol (will be removed from the
     * batch), false if the message should remain in the batch and be passed up.<p/> The default
     * implementation tries to find a header matching the current protocol's ID and returns true if
     * there is a match, or false otherwise
     */
