...
public class RouterStub extends ReceiverAdapter
    ...
    /**
     * Registers mbr with the GossipRouter under the given group, with the given logical name and
     * physical address. Establishes a connection to the GossipRouter and sends a CONNECT message.
     *
     ...
     */
    public void connect(String group, Address addr, String logical_name,
        PhysicalAddress phys_addr) throws Exception {
        synchronized (this) {
            _doConnect();
        }
        try {
    ...
    /**
     * Fetches a list of {@link PingData} from the GossipRouter, one for each member in the given
     * group. This call returns immediately and when the results are available, the {@link
     * org.jgroups.stack.RouterStub.MembersNotification#members(List)} callback will be invoked.
     *
    ...
    public void sendToAllMembers(String group, Address sender, byte[] data, int offset,
        int length) throws Exception {
        sendToMember(group, null, sender, data, offset, length); // null destination represents mcast
    }

    public void sendToMember(String group, Address dest, Address sender, byte[] data, int offset,
        int length) throws Exception {
        try {
