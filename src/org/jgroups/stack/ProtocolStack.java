...
public class ProtocolStack extends Protocol {
     ...
     * to the protocols above and below correctly and adjusts the linked list of protocols
     * accordingly. Note that this method may change the value of top_prot or bottom_prot.
     *
     * @param prot The protocol to be inserted. Before insertion, a sanity check will ensure that
     * none of the existing protocols have the same name as the new protocol.
     * @param position Where to place the protocol with respect to the neighbor_prot (ABOVE, BELOW)
     * @param neighbor_prot The name of the neighbor protocol. An exception will be thrown if this
     * name is not found
     * @throws Exception Will be thrown when the new protocol cannot be created, or inserted.
     */
    public void insertProtocol(Protocol prot, Position position,
        String neighbor_prot) throws Exception {
    ...
    public void insertProtocol(Protocol prot, Position position,
        Class<? extends Protocol> neighbor_prot) throws Exception {
        if (neighbor_prot == null)
            throw new IllegalArgumentException("neighbor_prot is null");
        Protocol neighbor = findProtocol(neighbor_prot);
        if (neighbor == null)
    ...
    @SafeVarargs
    public final void insertProtocol(Protocol prot, Position position,
        Class<? extends Protocol>... neighbor_prots) throws Exception {
