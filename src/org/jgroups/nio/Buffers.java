...
/**
 * Class to do scattering reads or gathering writes on a sequence of {@link ByteBuffer} instances.
 * The buffers are kept in an array with fixed capacity (max Short.MAX_VALUE). Buffers can be added
 * and removed dynamically (they're dropped when the capacity is exceeded).<p/> A read is successful
 * when all non-null buffers from left to right are filled, ie. all {@link ByteBuffer#remaining()}
 * methods return 0.<p/> Same for writes: when all non-null buffers (from left to right) have been
 * written ({@link ByteBuffer#remaining()} == 0), a write is considered successful; otherwise it is
 * partial.<p/> Individual buffers can be accessed; e.g. for reading its value after a read. It is
 * also possible to add buffers dynamically, e.g. after reading a 'length' buffer, a user may want
 * to add a new buffer allocated for reading 'length' bytes.<p/> This class is not synchronized.
 *
 * @author Bela Ban
 * @since 3.6.5
 */
...
public class Buffers implements Iterable<ByteBuffer> {
     ...
     * @return The data buffer (position is 0 and limit is length), or null if not all data could be
     * read.
     */
     ...
     * limit]. Returns true if the scattering read was successful, else false. Note that to read the
     * contents of the individual buffers, {@link ByteBuffer#clear()} has to be called (all buffers
     * have their position == limit on a successfull read).
     */
    // TODO: 17/5/25 by zmyer
    ...
    /**
     * Writes the buffers from position to limit to the given channel. Note that all buffers need to
     * have their {@link ByteBuffer#position} at the start of the data to be written be at the start
    ...
    /**
     * Copies the data that has not yet been written and moves last_copied. Typically done after an
     * unsuccessful write, if copying is required. This is typically needed if the output buffer is
     * reused. Note that direct buffers will be converted to heap-based buffers
     */
    // TODO: 17/5/25 by zmyer
    public Buffers copy() {
    ...
    /**
     * Looks at all buffers in range [position .. limit-1] and nulls buffers that have no remaining
     * data. Returns true if all buffers could be nulled, and false otherwise
     */
     ...
     * Direct buffers are converted to heap-based buffers
     */
    // TODO: 17/5/25 by zmyer
