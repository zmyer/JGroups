...
public class Channel implements Closeable {
    ...
    public ProtocolStack getProtocolStack() {
        return ch.getProtocolStack();
    }

    public SocketFactory getSocketFactory() {
        return ch.getProtocolStack().getBottomProtocol().getSocketFactory();
    }
     ...
     * <p>
     *
     * @param cluster_name The name of the channel to connect to.
     ...
     * <code>connect<code> and <code>getState<code> in succession.
     *
     ...
     *
     * @return true if channel is connected to cluster (group) and can send/receive messages, false
     * otherwise
     */
    ...
    /**
     * Determines whether the channel is in the connecting state; this means {@link
     * Channel#connect(String)} has been called, but hasn't returned yet
     *
     * @return true if the channel is in the connecting state, false otherwise
     */
    @ManagedAttribute public boolean isConnecting() {
        return ch.isConnecting();
    }
     ...
     *
     * @param dst Destination address for message. If null, message will be sent to all current
     * group members
     ...
     * send <code>length</code> bytes starting at <code>offset</code>
     ...
     * channel is implementation defined (may return null). Calling this method on a closed channel
     * returns null. Addresses can be used as destination in the <code>send()</code> operation.
     *
     ...
     * members in the cluster are not included in the flush. The list of flush participants should
     * be a proper subset of the current view.
     * <p/>
     * If this flush is not automatically resumed it is an obligation of the application to invoke
     * the matching {@link #stopFlush(List)} method with the same list of members used in {@link
     ...
     * allowed to send any messages until {@link #stopFlush()} is called.
     * <p/>
     * In the case of flush collisions (another member attempts flush at roughly the same time)
     * start flush will fail by throwing an Exception. Applications can re-attempt flushing after
     * certain back-off period.
     * <p/>
     ...
     * participants are unblocked and allowed to send new and pending messages. <p>
     *
     * It is an obligation of the application to invoke the matching {@link #startFlush(List,
     * boolean)} method with the same list of members prior to invocation of this method.
     *
