 ...
 * New members can be started; all existing members will update their graphical appearance to
 * reflect the new membership. When the coordinator itself is killed, another one will take over
 * (the next in rank).<p> A nice demo is to start a number of Topology instances at the same time.
 * All of them will be blue (all are coordinators since they don't find each other). Then the MERGE3
 * protocol sets in and only one will retain its coordinator role.
 *
 * @author Bela Ban
 */
